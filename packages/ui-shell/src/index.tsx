...
type SectionPanelOwnProps = PanelSurfaceOwnProps & {
  introClassName?: string
  eyebrow?: ReactNode
  title?: ReactNode
  description?: ReactNode
...
const APP_FRAME_PRESETS: Record<AppFramePreset, { maxWidth: string; gutter: string }> = {
  ...
  stage: { maxWidth: '80rem', gutter: '32px' },
}
...
export function SectionPanel<T extends ElementType = 'section'>({
  as,
  className,
  tone = 'default',
  padding = 'md',
  ...
  children,
  ...props
}: PolymorphicProps<T, SectionPanelOwnProps>) {
  return (
    <PanelSurface
      as={(as || 'section') as ElementType}
      className={joinClasses('ui-shell-section-panel', className)}
      padding={padding}
      tone={tone}
      {...(props as ComponentPropsWithoutRef<T>)}
    >
      {eyebrow || title || description ? (
        <SectionIntro
          className={joinClasses('ui-shell-section-panel__intro', introClassName)}
...
export function AppFrame<T extends ElementType = 'div'>({
  ...
  const frame = APP_FRAME_PRESETS[preset]
  const style = {
    '--ui-shell-max-width': maxWidth ?? frame.maxWidth,
    '--ui-shell-gutter': gutter ?? frame.gutter,
  } as CSSProperties
