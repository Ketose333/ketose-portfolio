...
export function GamePage() {
  ...
  const protocolItems = [
    ...
    { label: '퀵 타이밍', value: timingState },
    { label: '스택', value: `${surfaceState.stackEntries.length}개` },
    { label: '존 순서', value: '좌→우 M1~M3 / S1~S4' },
  ]
          ...
          <section className="board" aria-label="필드">
            ...
            <section className="game-toolbar game-toolbar--rail" aria-label="게임 조작">
              <div className="game-toolbar__identity">
                ...
                <div className="game-toolbar__status-grid" aria-label="듀얼 프로토콜">
                  {protocolItems.map((item) => (
                    <div className="game-toolbar__status-card" key={item.label}>
                      <span className="game-toolbar__status-label">{item.label}</span>
                      <strong className="game-toolbar__status-value">{item.value}</strong>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="game-toolbar__cluster">
                  ...
                  <div className="game-toolbar__group">
                    <ButtonSurface
                      ...
                      title="현재 스택 확인"
                      variant={surfaceState.stackActive ? 'solid' : 'neutral'}
                    >
                      스택 보기
                    </ButtonSurface>
                    <ButtonSurface
