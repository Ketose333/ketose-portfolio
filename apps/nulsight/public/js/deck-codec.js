(() => {
  ...
  function encodeV2FromCounts(counts) {
    const entries = Object.entries(counts || {}).filter(([, qty]) => Number.isInteger(qty) && qty > 0).map(([cardId, qty]) => [normalize(cardId), qty]).filter(([cardId]) => keyToIndex.has(cardId)).sort((a, b) => a[0].localeCompare(b[0]));
  ...
  // src/client/globals/deck-codec.ts
